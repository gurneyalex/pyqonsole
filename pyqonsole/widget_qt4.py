...
#
""" Provide the Widget class for PyQt4
...
raise Exception('qt4 not yet supported')
# XXX need more deep changes than expected
# XXX factorize code with qt3 widget
...
class Widget(Signalable, qt.QFrame):
    ...
    def __init__(self, qapp, parent=None):
        ...
        self.scrollbar.setCursor(qt.QCursor(Qt.ArrowCursor))
...
##         self._qapp.removeEventFilter( self )
    ...
    def setColorTable(self, table):
        ...
        palette.setColor(self.backgroundRole(),
                         self.color_table[DEFAULT_BACK_COLOR].color) 
        self.update()
    ...
    def setScroll(self, cursor, lines):
        ...
        self.scrollbar.setPageStep(self.lines)
...
#        self.setMicroFocusHint(xpos, ypos, 0, self.font_h)
    ...
    def setMouseMarks(self, on):
        self.mouse_marks = on
        if on:
            self.setCursor(qt.QCursor(Qt.IBeamCursor))
    ...
    def eventFilter(self, obj, e):
        ...
        we have focus.
        """
        if (e.type() == QEvent.Shortcut) and self._qapp.focusWidget() == self:
            e.ignore()
            return True
...
##                 paint.setBackgroundColor(bColor)
    ...
    def focusOutEvent(self, ev):
        """don't erase area"""
        self.repaint(self._cursor_rect)
