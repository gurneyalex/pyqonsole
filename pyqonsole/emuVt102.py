...
# Tokens
TY_CHR___ = 0
def TY_CTL___(A):      return                             ((ord(A) & 0xff) << 8) | 1
def TY_ESC___(A):      return                             ((ord(A) & 0xff) << 8) | 2
...
# Decoder
def lec(P, L, C): return p == P and s[L]  == C
def lun():        return p == 1 and cc >= 32
def les(P, L, C): return p == P and s[L] < 256 and (self.__tbl[s[L]] & C) == C
def eec(C):       return p >= 3 and cc == C
def ees(C):       return p >= 3 and cc < 256 and (tbl[cc] & C) == C
def eps(C):       return p >= 3 and s[2] != ord('?') and s[2] != ord('>') and cc < 256 and (self.__tbl[cc] & C) == C
def epp():        return p >= 3 and s[2] == ord('?')
def egt():        return p >= 3 and s[2] == ord('>')
def Xpe():        return len(self.__pbuf) >= 2 and self.__pbuf[1] == ord(']') 
def Xte():        return Xpe() and cc == 7
def ces(C):       return cc < 256 and (self.__tbl[cc] & C) == C and not Xte()
...
class EmuVt102(Emulation):
    ...
    def onRcvChar(self, cc):
        ...
        if self.getMode(MODE_Ansi): # Decide on proper action
            if lec(1, 0, ESC):
                ...
                self.__tau(TY_CHR___, self._applyCharset(cc), 0)
                self.__resetToken()
            elif lec(2, 0, ESC):
                self.__tau(TY_ESC___(s[1]), 0, 0)
                self.__resetToken()
            elif les(3, 1, SCS):
                self.__tau(TY_ESC_CS(s[1], s[2]), 0, 0)
                self.__resetToken()
            elif lec(3, 1, ord('#')):
                tau(TY_ESC_DE(s[2]), 0, 0)
                resetToken()
            elif eps(CPN):
                self.__tau(TY_CSI_PN(cc), argv[0], argv[1])
                self.__resetToken()
            elif ees(DIG):
                ...
                for argv in argv:
                    if epp():
                        tau(TY_CSI_PR(cc, arg), 0, 0)
                    elif egt():
                        tau(TY_CSI_PG(cc), 0, 0) # spec. elif token == for ESC]>0c or ESC]>c
                    else:
                        tau(TY_CSI_PS(cc, arg), 0, 0)
                self.__resetToken()
            ...
            if lec(1, 0, ESC):
                ...
                self.__tau(TY_VT52__(s[1]), 0, 0)
                self.__resetToken()
            else:
                self.__tau(TY_VT52__(s[1]), s[2], s[3])
                self.__resetToken()
    ...
    def tau(self, token, p, q ):
        if token == TY_CHR___: self._scr.showCharacter(p) # UTF16
    
        # 127 DEL: ignored on input
    
        elif token == TY_CTL___('@') : pass # NUL: ignored
        elif token == TY_CTL___('A') : pass # SOH: ignored
        elif token == TY_CTL___('B') : pass # STX: ignored
        elif token == TY_CTL___('C') : pass # ETX: ignored
        elif token == TY_CTL___('D') : pass # EOT: ignored
        elif token == TY_CTL___('E') : self.__reportAnswerBack() # VT100
        elif token == TY_CTL___('F') : pass # ACK: ignored
        elif token == TY_CTL___('G') and self._connected: # VT100
            self._gui.bell()
            self.emit(qt.PYSIGNAL("notifySessionState(int)"), NOTIFYBELL)
            
        elif token == TY_CTL___('H') : self._scr.backSpace() # VT100
        elif token == TY_CTL___('I') : self._scr.tabulate()  # VT100
        elif token == TY_CTL___('J') : self._scr.newLine()   # VT100
        elif token == TY_CTL___('K') : self._scr.newLine()   # VT100
        elif token == TY_CTL___('L') : self._scr.newLine()   # VT100
        elif token == TY_CTL___('M') : self._scr.return_()   # VT100
        elif token == TY_CTL___('N') : self._useCharset(1)   # VT100
        elif token == TY_CTL___('O') : self._useCharset(0)   # VT100
        
        elif token == TY_CTL___('P')  : pass # DLE: ignored
        elif token == TY_CTL___('Q')  : pass # DC1: XON continue # VT100
        elif token == TY_CTL___('R')  : pass # DC2: ignored
        elif token == TY_CTL___('S')  : pass # DC3: XOFF halt # VT100
        elif token == TY_CTL___('T')  : pass # DC4: ignored
        elif token == TY_CTL___('U')  : pass # NAK: ignored
        elif token == TY_CTL___('V')  : pass # SYN: ignored
        elif token == TY_CTL___('W')  : pass # ETB: ignored
        elif token == TY_CTL___('X')  : self._scr.showCharacter(0x2592) # VT100
        elif token == TY_CTL___('Y')  : pass # EM : ignored
        elif token == TY_CTL___('Z')  : self._scr.showCharacter(0x2592) # VT100
        elif token == TY_CTL___('[')  : pass # ESC: cannot be seen here.
        elif token == TY_CTL___('\\') : pass # FS : ignored 
        elif token == TY_CTL___(']')  : pass # GS : ignored
        elif token == TY_CTL___('^')  : pass # RS : ignored
        elif token == TY_CTL___('_')  : pass # US : ignored
    
        elif token == TY_ESC___('D') : self._scr.index() # VT100
        elif token == TY_ESC___('E') : self._scr.NextLine() # VT100
        elif token == TY_ESC___('H') : self._scr.changeTabStop(TRUE) # VT100
        elif token == TY_ESC___('M') : self._scr.reverseIndex() # VT100
        elif token == TY_ESC___('Z') : self.__reportTerminalType()
        elif token == TY_ESC___('c') : self.reset()
    
        elif token == TY_ESC___('n') : self._useCharset(2)
        elif token == TY_ESC___('o') : self._useCharset(3)
        elif token == TY_ESC___('7') : self._saveCursor()
        elif token == TY_ESC___('8') : self._restoreCursor()
    
        elif token == TY_ESC___('=') : self.setMode(MODE_AppKeyPad)
        elif token == TY_ESC___('>') : self.resetMode(MODE_AppKeyPad)
        elif token == TY_ESC___('<') : self.setMode(MODE_Ansi) # VT100
        ...
        elif token == TY_CSI_PS('h',   4): self._scr.setMode(MODE_Insert)
        elif token == TY_CSI_PS('h',  20): self.setMode(MODE_NewLine)
        elif token == TY_CSI_PS('i',   0): pass # IGNORE: attached printer # VT100
        elif token == TY_CSI_PS('i',   4): pass # IGNORE: attached printer # VT100
        elif token == TY_CSI_PS('i',   5): self.__setPrinterMode(true)     # VT100
        elif token == TY_CSI_PS('l',   4): self._scr.resetMode(MODE_Insert)
        elif token == TY_CSI_PS('l',  20): self.resetMode(MODE_NewLine)
        elif token == TY_CSI_PS('s',   0): self._saveCursor()
        elif token == TY_CSI_PS('u',   0): self._restoreCursor()
    
        elif token == TY_CSI_PS('m',   0): self._scr.setDefaultRendition()
        elif token == TY_CSI_PS('m',   1): self._scr.  setRendition(RE_BOLD)      # VT100
        elif token == TY_CSI_PS('m',   4): self._scr.  setRendition(RE_UNDERLINE) # VT100
        elif token == TY_CSI_PS('m',   5): self._scr.  setRendition(RE_BLINK)     # VT100
        elif token == TY_CSI_PS('m',   7): self._scr.  setRendition(RE_REVERSE)
        ...
        elif token == TY_CSI_PS('m',  22): self._scr.resetRendition(RE_BOLD)
        elif token == TY_CSI_PS('m',  24): self._scr.resetRendition(RE_UNDERLINE)
        elif token == TY_CSI_PS('m',  25): self._scr.resetRendition(RE_BLINK)
        elif token == TY_CSI_PS('m',  27): self._scr.resetRendition(RE_REVERSE)
        ...
        elif token == TY_CSI_PS('n', 5): self.__reportStatus()
        elif token == TY_CSI_PS('n', 6): self.__reportCursorPosition()
        elif token == TY_CSI_PS('q', 0): pass # IGNORED: LEDs off # VT100
        elif token == TY_CSI_PS('q', 1): pass # IGNORED: LED1 on  # VT100
        elif token == TY_CSI_PS('q', 2): pass # IGNORED: LED2 on  # VT100
        elif token == TY_CSI_PS('q', 3): pass # IGNORED: LED3 on  # VT100
        elif token == TY_CSI_PS('q', 4): pass # IGNORED: LED4 on  # VT100
        elif token == TY_CSI_PS('x', 0):self.__reportTerminalParms(2) # VT100
        elif token == TY_CSI_PS('x', 1):self.__reportTerminalParms(3) # VT100
        ...
        elif token == TY_CSI_PN('c'): self.__reportTerminalType() # VT100
        elif token == TY_CSI_PN('d'): self._scr.setCursorY(p)     # LINUX
        elif token == TY_CSI_PN('f'): self._scr.setCursorYX(p, q) # VT100
        elif token == TY_CSI_PN('r'): self._setMargins(p, q)      # VT100
        elif token == TY_CSI_PN('y'): pass # IGNORED: Confidence test # VT100
        ...
        elif token == TY_CSI_PR('h',  5): self._scr.setMode(MODE_Screen)   # VT100
        elif token == TY_CSI_PR('l',  5): self._scr.resetMode(MODE_Screen) # VT100
    
        elif token == TY_CSI_PR('h',  6): self._scr.setMode(MODE_Origin)     # VT100
        elif token == TY_CSI_PR('l',  6): self._scr.resetMode(MODE_Origin)   # VT100
        elif token == TY_CSI_PR('s',  6): self._scr.saveMode(MODE_Origin)    # FIXME
        elif token == TY_CSI_PR('r',  6): self._scr.restoreMode(MODE_Origin) # FIXME
    
        elif token == TY_CSI_PR('h',  7): self._scr.setMode(MODE_Wrap)     # VT100
        elif token == TY_CSI_PR('l',  7): self._scr.resetMode(MODE_Wrap)   # VT100
        elif token == TY_CSI_PR('s',  7): self._scr.saveMode(MODE_Wrap)    # FIXME
        elif token == TY_CSI_PR('r',  7): self._scr.restoreMode(MODE_Wrap) # FIXME
        ...
        elif token == TY_CSI_PR('h', 25): self.setMode(MODE_Cursor)   # VT100
        elif token == TY_CSI_PR('l', 25): self.resetMode(MODE_Cursor) # VT100
        ...
        elif token == TY_VT52__('Z'): self.__reportTerminalType()        # VT52
        ...
        elif token == TY_CSI_PG('c') : self.__reportSecondaryAttributes() # VT100
    
        else:
            self.__reportErrorToken();
    ...
    def reportAnswerBack(self):
        ANSWER_BACK = os.path.expandvars("ANSWER_BACK")
        if ANSWER_BACK == "$ANSWER_BACK":
            self.sendString(" ")
        else:
            self.sendString(ANSWER_BACK)
    ...
    def _resetCharset(self, scrno):
        self._charset[scrno].cuCs = 0
        self._charset[scrno].charset = "BBBB"
        self._charset[scrno].saGraphic = False
        self._charset[scrno].saPound = False
        self._charset[scrno].graphic = False
        self._charset[scrno].pound = False
        self._charset[scrno].trans = "[\\]{|}~"
        
    def _setCharset(self, n, cs):
        self._charset[0].charset[n & 3] = cs
        self._useCharset(self._charset[0].cuCs)
        self._charset[1].charset[n & 3] = cs
        self._useCharset(self._charset[1].cuCs)
    ...
    def _saveCursor(self):
        CHARSET = self._charset[self._scr is self._screen[1]]
        CHARSET.saGraphic = CHARSET.graphic
        CHARSET.saPound = CHARSET.pound
        CHARSET.saTrans = CHARSET.trans
        self._scr.saveCursor()
        
    def _restorCursor(self):
        CHARSET = self._charset[self._scr is self._screen[1]]
        CHARSET.Graphic = CHARSET.saGraphic
        CHARSET.Pound = CHARSET.saPound
        CHARSET.Trans = CHARSET.saTrans
        self._scr.restorCursor()
    ...
    def saveMode(self, m):
        self._saveParm['mode'][m] = self._currParm['mode'][m]
        
    def restoreMode(self, m):
        if self._saveParm['mode'][m]:
            self.setMode(m)
        else:
            self.resetMode(m)
        
    def getMode(self, m):
        return self._currParm['mode'][m]
