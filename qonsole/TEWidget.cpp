...
HCNT("setImage");
  ...
  if (resizing && terminalSizeHint)
  {
     ...
     if (!mResizeWidget)
     {
        ...
        mResizeLabel = new QLabel("Size: XXX x XXX", mResizeWidget);
        l->addWidget(mResizeLabel, 1, AlignCenter);
        mResizeWidget->setMinimumWidth(mResizeLabel->fontMetrics().width("Size: XXX x XXX")+20);
     ...
     QString sizeStr = QString("Size: %1 x %2").arg(columns).arg(lines);
